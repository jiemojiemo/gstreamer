...
int tutorial_main(int argc, char *argv[]) {
  ...
  data.my_filter = gst_element_factory_make("my_filter", "my_filter");
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.convert,
                   data.resample, data.my_filter, data.sink, NULL);
  if (!gst_element_link_many(data.convert, data.resample, data.my_filter, data.sink, NULL)) {
  ...
  g_object_set(
      data.source, "uri",
      "file:///Users/user/Documents/work/测试视频/video_1280x720_30fps_180sec.mp4",
      NULL);

  // set my filter parameters
  g_object_set(data.my_filter, "delay", 1000.0f, NULL);
  g_object_set(data.my_filter, "feedback", 0.5f, NULL);
